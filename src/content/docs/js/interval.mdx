...
There are three main ways to create `Interval` vectors from scratch. Directly, using the [class constructor](#class-constructor); using strings via the static methods [`Interval.fromName`](#fromname) or [`Interval.fromSPN`](#fromspn), or from two [`Pitch`](/js/pitch) vectors via [`Interval.between`](#between).
...
Multiplies an `Interval` vector by a scalar.
