...
There are three main ways to create `Pitch` vectors from scratch. Using the [class constructor](#class-constructor), or using the static methods [`Pitch.fromSPN`](#pitchfromspn) or [`Pitch.fromChroma`](#pitchfromchroma).
...
The [SPN](https://en.wikipedia.org/wiki/Scientific_pitch_notation) name of a `Pitch` as a string.

```ts
let p = Pitch.fromSPN("Abb5");
p.SPN; // "Abb5"
```
