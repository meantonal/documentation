...
There are three main ways to create `Pitch` vectors from scratch. Using the [class constructor](#class-constructor), using the static method [`Pitch.fromChroma`](#pitchfromchroma), or using static methods on a [parse class](/js/parse) to parse a `Pitch` from a string format like SPN or LilyPond.
...
Invert a `Pitch` about the passed in [`Axis`](/js/axis). An `Axis` is created from two `Pitches` that will be mapped to each other by the inversion across it. Returns the inverted Pitch as a new vector.
