...
```c
context = TonalContext.fromStrings("Eb", "major");
p = Pitch.fromDegree(2, -1, 4, context); // Gb4
```
...
```ts
audible(p: Pitch, T?: TuningMap): boolean;
```
...
Returns the nearest `Pitch` in a passed-in array to the calling `Pitch` instance. Uses an optional `TuningMap` to determine which `Pitch` is nearer than the others; if no `TuningMap` is passed in, defaults to 12TET.
