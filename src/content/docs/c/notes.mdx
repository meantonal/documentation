---
title: Notes
description: Reference page for functions that return Note vectors.
sidebar:
  order: 2
---

This page details all the functions that create and manipulate notes directly.
...
Predicate function that checks whether two `Note` vectors are equal. Note it does **not** check whether they are enharmonically equivalent: they must literally represent the same note in [SPN](https://en.wikipedia.org/wiki/Scientific_pitch_notation) to return `true`.
