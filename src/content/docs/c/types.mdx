...
The `MapVec` type exists to store the vectors used with a `Map1d` by [map_to_1d](/c/map/#map_to_1d) and `Map2d` by [map_to_2d](/c/map/#map_to_2d). Internally they are an alias for `Pitch`, much like `Interval` and `MirrorAxis`.
