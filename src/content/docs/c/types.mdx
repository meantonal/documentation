...
Created by [`create_edo_map`](/c/map/#create_edo_map). Used with [`pitch_to_number`](/c/pitch/#pitch_to_number) and [`pitches_compare`](/c/pitch/#pitches_compare).
...
The `LilyParseContext` type carries the state needed to parse a sequence of LilyPond pitches in relative mode: the previously-parsed `Pitch`, against which the next pitch's octave is resolved.
...
Created by [`lily_parse_context_init`](/c/parse/#lily_parse_context_init) and updated automatically by each call to [`pitch_from_relative_lily`](/c/parse/#pitch_from_relative_lily).
