...
Used with functions that operate on a `TonalContext`. Numbered in descending fifths, starting with Lydian as 0.
...
The `MapVec` type exists to store the vectors used with a `Map1d` by [map_to_1d](/c/map/#map_to_1d) and `Map2d` by [map_to_2d](/c/map/#map_to_2d).
