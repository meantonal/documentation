...
`Interval` vectors can obviously be initialised directly:
...
Parses two strings into `Note` vectors from [Scientific Pitch Notation](https://en.wikipedia.org/wiki/Scientific_pitch_notation) to generate an `Interval` vector from. Outputs via a passed in `Interval` pointer. Returns 1 if anything went wrong during parsing.
...
```c
Interval interval_between(Note p, Note q);
```

Creates an `Interval` from two `Note` vectors by subtracting the first from the second.
...
Predicate function that checks whether two `Interval` vectors are equal. Note that it does **not** check whether they are enharmonically equivalent: they must literally represent the same *vector*.
