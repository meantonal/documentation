...
Uses an [`EDOMap`](/c/types/#edomap) produced by [`create_edo_map`](/c/map/#creape_edo_map). Returns an ordered pitch numbering for the passed `Pitch` as an integer. For 12TET, this will be the ordinary MIDI value for a given `Pitch`, but for other EDO tunings it provides an ordered MIDI-equvalent mapping.
...
Uses an [`EDOMap`](/c/types/#edomap) produced by [`create_edo_map`](/c/map/#creape_edo_map). In the specified EDO:
