...
```c
Pitch pitch_from_degree(enum Degree degree, int alteration, int octave, TonalContext key);
```

Creates a `Pitch` vector from a specified scale degree (0-indexed, tonic is 0) and chromatic alteration (0 is diatonic) within a `TonalContext`, along with its octave number (using [SPN](https://en.wikipedia.org/wiki/Scientific_pitch_notation) octave numbering).
...
context_from_str("Eb", MAJOR, &key);
Pitch p = pitch_from_degree(MEDIANT, -1, 4, key); // Gb4
```
...
```c
int pitch_to_number(Pitch p, EDOMap T);
```

Uses an [`EDOMap`](/c/types/#edomap) produced by [`create_edo_map`](/c/map/#create_edo_map). Returns an ordered pitch numbering for the passed `Pitch` as an integer. For 12TET, this will be the ordinary MIDI value for a given `Pitch`, but for other EDO tunings it provides an ordered MIDI-equvalent mapping.

### `pitches_compare`

```c
int pitches_compare(Pitch p, Pitch q, EDOMap T);
```

Uses an [`EDOMap`](/c/types/#edomap) produced by [`create_edo_map`](/c/map/#create_edo_map). In the specified EDO:
...
EDOMap T;
create_edo_map(53, &T);
pitches_compare(p, q, T); // 1

create_edo_map(31, &T);
pitches_compare(p, q, T); // -1

create_edo_map(12, &T);
pitches_compare(p, q, T); // 0
...
Predicate function that checks whether a `Pitch`'s frequency, as rendered by the passed-in [`TuningMap`](/c/types/#tuningmap), falls within the approximate range of human hearing (20Hz to 20kHz, inclusive of both bounds).
...
TuningMap T;
tuning_map_from_edo(12, (Pitch){29, 11}, 440, &T);

pitch_audible(p, T); // true, A4 = 440Hz

pitch_from_spn("C-1", &p);
pitch_audible(p, T); // false, C-1 (MIDI 0) is ~8.18Hz, below the 20Hz floor
```
...
tuning_map_from_edo(31, (Pitch){29, 11}, 440, &T);
