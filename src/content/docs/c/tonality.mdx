...
```c
TonalContext key;
if (context_from_str(str, MAJOR, &key)) {
...
```c
int degree_chroma(enum Degree degree, int alteration, TonalContext key);
```

Produces the chroma (signed distance from C measured in perfect 5ths) of the variant of the passed-in scale degree (0-indexed so the tonic is 0) that matches the specified alteration with respect to the passed-in `TonalContext`.
