...
This page details functions that build and operate on the generic `PitchClassSet`, where a "pitch class" is taken to mean any unique letter name + accidental combination (so $\sf{D}\flat$ and $\sf{C}\sharp$ are not the same pitch class in meantonal, even if they are equivalent in the more common 12-tone pitch class sets).
...
```c
PitchClassSet s = NULL;
s = pc_set_insert(s, 3); // adds "A" to the set
```
