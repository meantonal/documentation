...
<Aside type="note" title="Two different `bool` conventions">

`pitch_spn` and `pitch_helmholtz` can render arbitrarily large accidentals as text (e.g. "5#"), so a `true` return from them is just a warning that the accidental is unusually large — the name is still rendered in full. `pitch_lily` and `pitch_abc` have no notation at all beyond a double sharp/flat, so for them `true` means rendering was refused outright, and `"ERR"` was written to `out` instead.
