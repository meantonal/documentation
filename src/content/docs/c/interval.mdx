...
Note: this is **not** the same as "inverting" an interval at the octave. That operation would be:
...
Adds two `Interval` vectors to produce a new `Interval`. To take a difference $m - n$, prefer
