...
Returns true if an `Interval` is capable of occurring diatonically (i.e. is found within the major scale).
...
Returns true if an `Interval` is capable of occurring in a tonal context, including via chromatically altered degrees (i.e. is comprised of notes at most a half step away from notes in a shared diatonic scale).
...
Multiplies an `Interval` vector by an integer scalar.
