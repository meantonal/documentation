...
Internally, the C implementation of Meantonal defines the [`Interval`](/c/types/#interval) type as a struct identical to the [`Pitch`](/c/types/#pitch) type, by simply alisasing `Pitch`:

```c
typedef Pitch Interval;
```

The TypeScript implementation defines the [`Interval`](/js/interval) Class:
