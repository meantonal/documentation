...
Much like [notes](/learn/notes), Meantonal stores intervals as two-dimensional vectors. In fact, intervals are simply differences between two notes' vectors:
...
Internally, the [`Interval`](/c/types/#interval) type is a struct identical to the `Note` type:
