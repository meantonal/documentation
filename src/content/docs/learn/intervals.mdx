...
5. Add the correct number of chromatic semitones to arrive at the correct _quality_.
   ...
   | P or M | $0$                                                 |
   |   m    | $-1$                                                |
   |   d    | $-1$ if the size is $0$, $3$ or $4$; otherwise $-2$ |
   |   dd   | $-2$ if the size is $0$, $3$ or $4$; otherwise $-3$ |
   | _etc._ |                                                     |
...
Since we start from a major scale, in which all intervals above the tonic form major or perfect intervals, the adjustments we have to make to account for diminished intervals, along with doubly-diminished and beyond, are slightly different for intervals in the first group vs. intervals in the second.

In his book *Moveable Counterpoint in the Strict Style*, the composer [Sergei Taneyev](https://en.wikipedia.org/wiki/Sergei_Taneyev) calls intervals in the first group ¹Ints and intervals in the second group ²Ints, and the distinction has relevance to invertible counterpoint.
