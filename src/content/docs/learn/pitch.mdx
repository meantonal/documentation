---
title: Pitch
description: An overview of how Meantonal constructs pitches
sidebar:
  order: 1
---
...
Meantonal stores pitches as two-dimensional vectors. C₋₁ is defined to be $(0, 0)$ to conform to 0 representing the [MIDI value](https://en.wikipedia.org/wiki/MIDI#Messages) for the same pitch.
...
Internally, the C implementation of Meantonal defines the [`Pitch`](/c/types/#pitch) type as a simple struct:
...
The TypeScript implementation defines the [`Pitch`](/js/pitch) Class:
...
To help understand what this means, let's see how we would represent the pitches of a C major scale from C₋₁ to C₀, given the [familiar formula](https://en.wikipedia.org/wiki/Major_scale#Structure) of WWHWWWH:
...
| :-----------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: |
|               |               |               |               |               |  $\sf{C}_0$   |
|               |               | $\sf{F}_{-1}$ | $\sf{G}_{-1}$ | $\sf{A}_{-1}$ | $\sf{B}_{-1}$ |
| $\sf{C}_{-1}$ | $\sf{D}_{-1}$ | $\sf{E}_{-1}$ |               |               |               |

<Aside type="tip">

D♭₋₁ and C♯₋₁ are represented by different vectors:

- D♭₋₁ is a diatonic semitone _above_ C₋₁ at $(0,1)$
- C♯₋₁ is a diatonic semitone _below_ D₋₁ at $(1,-1)$
...
To put things in perspective: $(-125,-50)$ and $(125, 50)$ represent C₋₂₆ and C₂₄ and would be well-represented by the `Pitch` type even if it were capped at 8-bit integers. That's eome **fifty octaves** of range, or from roughly **240nHz** to **264MHz**.
...
$O$ represents the octave. Since we want $(0, 0)$ to represent C₋₁, the value of $O$ is greater than the octave component of a pitch's SPN value by 1.
