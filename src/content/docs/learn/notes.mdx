...
- The first component of a note's vector represents increments in **whole steps**.
- The second component represents increments in **diatonic half steps** (that is, from C to D♭, not from C to C♯).
...
Although the abstract vector space of whole steps and half steps shown above is theoretically infinite, Meantonal represents these coordinates with 8-bit signed integers, meaning each coordinate's value has a range of $[-128,127]$.

Just how big is this, though? Do we cover as many octaves as MIDI at least?

To put things in perspective: $(-125,-50)$ and $(125, 50)$ represent C₋₂₆ and C₂₄ and are well-represented by the `Note` type. That's eome **fifty octaves** of range, or from roughly **240nHz** to **264MHz**.

Suffice to say: you're not going to run out of notes.
